...
void Redis::observer_channel_message(){
    ...
    while(REDIS_OK == redisGetReply(this->_subscribe_context, (void **)&replay)){
        //订阅接收到的消息是一个带三元素的数组
        if(replay != nullptr && replay->element[2] != nullptr && replay->element[2]->str != nullptr){
            //给业务层上报通道上发生的消息
            _notify_message_handler(atoi(replay->element[1]->str), replay->element[2]->str);
        }
